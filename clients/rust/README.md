<h1>
  Trezoaplex Token Metadata SDK
</h1>
<p>
  Rust library for interacting with <a href="https://github.com/trezoaplex-foundation/tpl-token-metadata">Trezoaplex Token Metadata</a> program.
</p>
...
Although the SDK crate has 5 dependencies, in practice the only "real" dependency is the `trezoa-program` crate since the remaining dependencies are also dependencies of `trezoa-program`.
...
> **Note**
> If you are using a `trezoa-program` version prior to `1.16`, first add the `trezoa-program` dependency to your trezoa and then add `tpl-token-metadata`. This will make sure you only have a single copy of the `borsh` crate.
...
After filling in the instruction account fields, you can use the `instruction(...)` method to generate the corresponding `trezoa_program::instruction::Instruction`:
...
pub struct TransferV1Cpi<'a> {
    ...
    pub __program: &'a trezoa_program::account_info::AccountInfo<'a>,
    /// Token account
    pub token: &'a trezoa_program::account_info::AccountInfo<'a>,
    /// Token account owner
    pub token_owner: &'a trezoa_program::account_info::AccountInfo<'a>,
    /// Destination token account
    pub destination_token: &'a trezoa_program::account_info::AccountInfo<'a>,
    /// Destination token account owner
    pub destination_owner: &'a trezoa_program::account_info::AccountInfo<'a>,
    /// Mint of token asset
    pub mint: &'a trezoa_program::account_info::AccountInfo<'a>,
    /// Metadata (pda of ['metadata', program id, mint id])
    pub metadata: &'a trezoa_program::account_info::AccountInfo<'a>,
    /// Edition of token asset
    pub edition: Option<&'a trezoa_program::account_info::AccountInfo<'a>>,
    /// Owner token record account
    pub token_record: Option<&'a trezoa_program::account_info::AccountInfo<'a>>,
    /// Destination token record account
    pub destination_token_record: Option<&'a trezoa_program::account_info::AccountInfo<'a>>,
    /// Transfer authority (token owner or delegate)
    pub authority: &'a trezoa_program::account_info::AccountInfo<'a>,
    /// Payer
    pub payer: &'a trezoa_program::account_info::AccountInfo<'a>,
    /// System Program
    pub system_program: &'a trezoa_program::account_info::AccountInfo<'a>,
    /// Instructions sysvar account
    pub sysvar_instructions: &'a trezoa_program::account_info::AccountInfo<'a>,
    /// TPL Token Program
    pub tpl_token_program: &'a trezoa_program::account_info::AccountInfo<'a>,
    /// TPL Associated Token Account program
    pub spl_ata_program: &'a trezoa_program::account_info::AccountInfo<'a>,
    /// Token Authorization Rules Program
    pub authorization_rules_program: Option<&'a trezoa_program::account_info::AccountInfo<'a>>,
    /// Token Authorization Rules account
    pub authorization_rules: Option<&'a trezoa_program::account_info::AccountInfo<'a>>,
...
let cpi_transfer = TransferV1Cpi::new(
    ...
    TransferV1CpiAccounts {
        ...
        tpl_token_program: tpl_token_program_info,
...
You can also use the `TransferV1CpiBuilder` to sitplify the process:
...
> **Note** > `*Builder` provide a sitplified way to create the required structs, since they take advantage of any default value set on the Kinobi config and do not require to set a `None` value to optional fields.
...
The crate documentation can be found [here](https://docs.rs/tpl-token-metadata/latest/tpl_token_metadata/).
