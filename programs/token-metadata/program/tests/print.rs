...
mod print {
    ...
    #[tokio::test]
    async fn success(tpl_token_program: Pubkey) {
    ...
    #[test_case::test_case(tpl_token_2022::id() ; "Token-2022 Program")]
    #[tokio::test]
    async fn pnft_editions_rule_set_inheritance_success(tpl_token_program: Pubkey) {
        let mut context = program_test().start_with_context().await;
    ...
    #[test_case::test_case(tpl_token_2022::id() ; "Token-2022 Program")]
    #[tokio::test]
    async fn fail_invalid_token_program(tpl_token_program: Pubkey) {
        let mut context = program_test().start_with_context().await;
    ...
    #[test_case::test_case(tpl_token_2022::id() ; "Token-2022 Program")]
    #[tokio::test]
    async fn fail_edition_already_initialized(tpl_token_program: Pubkey) {
        let mut context = program_test().start_with_context().await;
        let mut asset = DigitalAsset::default();
        asset
            .create_and_mint_with_supply(
    ...
    #[test_case::test_case(tpl_token_2022::id() ; "Token-2022 Program")]
    #[tokio::test]
    async fn fail_to_mint_edition_override_0(tpl_token_program: Pubkey) {
        let mut context = program_test().start_with_context().await;
        ...
        let test_master_edition = MasterEditionV2::new_from_asset(&asset);
        let test_edition_marker =
            EditionMarker::new_from_asset(&asset, &test_master_edition, 0, tpl_token_program);
    ...
    #[test_case::test_case(tpl_token_2022::id() ; "Token-2022 Program")]
    #[tokio::test]
    async fn fail_to_mint_edition_num_zero(tpl_token_program: Pubkey) {
    ...
    #[test_case::test_case(tpl_token_2022::id() ; "Token-2022 Program")]
    #[tokio::test]
    async fn increment_master_edition_supply(tpl_token_program: Pubkey) {
        let mut context = program_test().start_with_context().await;
        let mut slot = 1;
        ...
        let print_edition =
            EditionMarker::new_from_asset(&original_nft, &master_edition, 5, tpl_token_program);
        print_edition.create_from_asset(&mut context).await.unwrap();
        let _result = context.warp_to_slot(slot);
        slot += 1;
        ...
        let print_edition =
            EditionMarker::new_from_asset(&original_nft, &master_edition, 4, tpl_token_program);
        print_edition.create_from_asset(&mut context).await.unwrap();
        let _result = context.warp_to_slot(slot);
        slot += 1;
        ...
        let print_edition =
            EditionMarker::new_from_asset(&original_nft, &master_edition, 10, tpl_token_program);
        print_edition.create_from_asset(&mut context).await.unwrap();
        let _result = context.warp_to_slot(slot);
        ...
        let print_edition =
            EditionMarker::new_from_asset(&original_nft, &master_edition, 6, tpl_token_program);
        print_edition.create_from_asset(&mut context).await.unwrap();
    ...
    #[test_case::test_case(tpl_token_2022::id() ; "Token-2022 Program")]
    #[tokio::test]
    async fn cannot_mint_edition_num_higher_than_max_supply(tpl_token_program: Pubkey) {
        let mut context = program_test().start_with_context().await;
        ...
        let print_edition =
            EditionMarker::new_from_asset(&original_nft, &master_edition, 11, tpl_token_program);
        ...
        let print_edition =
            EditionMarker::new_from_asset(&original_nft, &master_edition, 999, tpl_token_program);
        let err = print_edition
            .create_from_asset(&mut context)
            .await
            .unwrap_err();
    ...
    #[test_case::test_case(tpl_token_2022::id() ; "Token-2022 Program")]
    #[tokio::test]
    async fn cannot_remint_existing_edition(tpl_token_program: Pubkey) {
        let mut context = program_test().start_with_context().await;
        ...
        let edition_99 =
            EditionMarker::new_from_asset(&original_nft, &master_edition, 99, tpl_token_program);
    ...
    #[test_case::test_case(tpl_token_2022::id() ; "Token-2022 Program")]
    #[tokio::test]
    async fn can_mint_out_missing_editions(tpl_token_program: Pubkey) {
        ...
        for i in 1..10 {
            ...
            let print_edition =
                EditionMarker::new_from_asset(&original_nft, &master_edition, i, tpl_token_program);
            print_edition.create_from_asset(&mut context).await.unwrap();
        }
