#![cfg(feature = "test-bpf")]
pub mod utils;

use trezoa_program_test::*;
use trezoa_sdk::{instruction::InstructionError, signature::Signer, transaction::TransactionError};
use utils::*;

mod mint {

    use tpl_utils::token::unpack;
    use num_traits::FromPrimitive;
    use trezoa_program::pubkey::Pubkey;
    use tpl_token_2022::state::Account;
    use token_metadata::{error::MetadataError, state::TokenStandard};

    use super::*;

    #[test_case::test_case(tpl_token::id() ; "Token Program")]
    #[test_case::test_case(tpl_token_2022::id() ; "Token-2022 Program")]
    #[tokio::test]
    async fn mint_programmable_nonfungible(tpl_token_program: Pubkey) {
        let mut context = program_test().start_with_context().await;

        // asset

        let mut asset = DigitalAsset::default();
        asset
            .create(
                &mut context,
                TokenStandard::ProgrammableNonFungible,
                None,
                tpl_token_program,
            )
            .await
            .unwrap();

        // mints one token

        let payer_pubkey = context.payer.pubkey();

        asset
            .mint(&mut context, None, None, 1, tpl_token_program)
            .await
            .unwrap();

        // asserts

        let account = get_account(&mut context, &asset.token.unwrap()).await;
        let token_account = unpack::<Account>(&account.data).unwrap().base;

        assert!(token_account.is_frozen());
        assert_eq!(token_account.amount, 1);
        assert_eq!(token_account.mint, asset.mint.pubkey());
        assert_eq!(token_account.owner, payer_pubkey);
    }

    #[test_case::test_case(tpl_token::id() ; "Token Program")]
    #[test_case::test_case(tpl_token_2022::id() ; "Token-2022 Program")]
    #[tokio::test]
    async fn mint_nonfungible(tpl_token_program: Pubkey) {
        let mut context = program_test().start_with_context().await;

        // asset

        let mut asset = DigitalAsset::default();
        asset
            .create(
                &mut context,
                TokenStandard::NonFungible,
                None,
                tpl_token_program,
            )
            .await
            .unwrap();

        // mints one token

        asset
            .mint(&mut context, None, None, 1, tpl_token_program)
            .await
            .unwrap();

        assert!(asset.token.is_some());

        if let Some(token) = asset.token {
            let account = get_account(&mut context, &token).await;
            let token_account = unpack::<Account>(&account.data).unwrap().base;

            assert!(!token_account.is_frozen());
            assert_eq!(token_account.amount, 1);
            assert_eq!(token_account.mint, asset.mint.pubkey());
            assert_eq!(token_account.owner, context.payer.pubkey());
        }
    }

    #[test_case::test_case(tpl_token::id() ; "Token Program")]
    #[test_case::test_case(tpl_token_2022::id() ; "Token-2022 Program")]
    #[tokio::test]
    async fn mint_fungible(tpl_token_program: Pubkey) {
        let mut context = program_test().start_with_context().await;

        // asset

        let mut asset = DigitalAsset::default();
        asset
            .create(
                &mut context,
                TokenStandard::Fungible,
                None,
                tpl_token_program,
            )
            .await
            .unwrap();

        // mints one token

        asset
            .mint(&mut context, None, None, 100, tpl_token_program)
            .await
            .unwrap();

        assert!(asset.token.is_some());

        if let Some(token) = asset.token {
            let account = get_account(&mut context, &token).await;
            let token_account = unpack::<Account>(&account.data).unwrap().base;

            assert!(!token_account.is_frozen());
            assert_eq!(token_account.amount, 100);
            assert_eq!(token_account.mint, asset.mint.pubkey());
            assert_eq!(token_account.owner, context.payer.pubkey());
        }
    }

    #[test_case::test_case(tpl_token::id() ; "Token Program")]
    #[test_case::test_case(tpl_token_2022::id() ; "Token-2022 Program")]
    #[tokio::test]
    async fn mint_fungible_asset(tpl_token_program: Pubkey) {
        let mut context = program_test().start_with_context().await;

        // asset

        let mut asset = DigitalAsset::default();
        asset
            .create(
                &mut context,
                TokenStandard::FungibleAsset,
                None,
                tpl_token_program,
            )
            .await
            .unwrap();

        // mints one token

        asset
            .mint(&mut context, None, None, 50, tpl_token_program)
            .await
            .unwrap();

        assert!(asset.token.is_some());

        if let Some(token) = asset.token {
            let account = get_account(&mut context, &token).await;
            let token_account = unpack::<Account>(&account.data).unwrap().base;

            assert!(!token_account.is_frozen());
            assert_eq!(token_account.amount, 50);
            assert_eq!(token_account.mint, asset.mint.pubkey());
            assert_eq!(token_account.owner, context.payer.pubkey());
        }
    }

    #[test_case::test_case(tpl_token::id() ; "Token Program")]
    #[test_case::test_case(tpl_token_2022::id() ; "Token-2022 Program")]
    #[tokio::test]
    async fn try_mint_multiple_programmable_nonfungible(tpl_token_program: Pubkey) {
        let mut context = program_test().start_with_context().await;

        let mut asset = DigitalAsset::default();
        let error = asset
            .create_and_mint(
                &mut context,
                TokenStandard::ProgrammableNonFungible,
                None,
                None,
                2,
                tpl_token_program,
            )
            .await
            .unwrap_err();

        assert_custom_error_ix!(1, error, MetadataError::EditionsMustHaveExactlyOneToken);
    }

    #[test_case::test_case(tpl_token::id() ; "Token Program")]
    #[test_case::test_case(tpl_token_2022::id() ; "Token-2022 Program")]
    #[tokio::test]
    async fn try_mint_multiple_nonfungible(tpl_token_program: Pubkey) {
        let mut context = program_test().start_with_context().await;

        let mut asset = DigitalAsset::default();
        let error = asset
            .create_and_mint(
                &mut context,
                TokenStandard::NonFungible,
                None,
                None,
                2,
                tpl_token_program,
            )
            .await
            .unwrap_err();

        assert_custom_error_ix!(1, error, MetadataError::EditionsMustHaveExactlyOneToken);
    }
}
